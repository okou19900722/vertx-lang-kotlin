...
/**
 * A function providing a DSL for building [StompServerOptions] objects.
 *
 *  STOMP Server options. You can also configure the Net Server used by the STOMP server from these
    options.
 *
 *
 * <p/>
 * NOTE: This function has been automatically generated from the [StompServerOptions original] using
    Vert.x codegen.
 *
 ...
 * @param idleTimeout  Set the idle timeout, default time unit is seconds. Zero means don't timeout.
    ...
    timeout. If you want change default time unit, use [io.vertx.core.net.NetServerOptions]
 ...
 * @param maxFrameInTransaction  Sets the maximum number of frame that can be added in a
    transaction. If the number of frame added to a transaction exceeds this threshold, the client
    receives an <code>ERROR</code> frame and is disconnected. The default is 1000.
 * @param maxHeaderLength  Sets the max header length.
 * @param maxHeaders  Sets the maximum number of headers. 1000 by default.
 * @param maxSubscriptionsByClient  Sets the maximum of subscriptions a client is allowed to
    register. If a client exceeds this number, it receives an error and the connection is closed.
 ...
 * @param secured  Enables or disables the server security feature. It requires an
    [io.vertx.ext.auth.AuthProvider] handler.
 * @param sendBufferSize  Set the TCP send buffer size
 * @param sendErrorOnNoSubscriptions  Sets whether or not an error is sent to the client when this
    client sends a message to an not subscribed destinations (no subscriptions on this destination).
 ...
 * @param supportedVersions  Sets the STOMP protocol versions supported by the server. Versions must
    be given in the decreasing order.
 ...
 * @param transactionChunkSize  Sets the chunk size when replaying a transaction. To avoid blocking
    the event loop for too long, large transactions are split into chunks, replayed one by one. This
    settings sets the chunk size.
 ...
 * @param websocketPath  Sets the websocket path. Only frames received on this path would be
    considered as STOMP frame.
 */
